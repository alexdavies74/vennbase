...
export interface CoveDBOptions<Schema extends DbSchema = DbSchema> {
  schema: Schema;
  backend?: BackendClient;
  fetchFn?: typeof fetch;
  appBaseUrl?: string;
  identityProvider?: () => Promise<CoveDBUser>;
...
const INTERNAL_USER_SCOPE_ROW_KEY = "__covedb_user_scope_v1__";
type LocalMutationListener = () => void;

export class CoveDB<Schema extends DbSchema = DbSchema> implements RowHandleBackend<Schema> {
  ...
  constructor(private readonly options: CoveDBOptions<Schema>) {
  ...
  async whoAmI(): Promise<CoveDBUser> {
