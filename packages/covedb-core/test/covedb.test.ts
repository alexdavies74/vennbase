...
import { CoveDBError } from "../src/errors";
...
function workerMetadataKey(kind: "runtime" | "url", hostHash: string): string {
  return `covedb:federation-worker-${kind}:v2:owner:${hostHash}`;
}
...
describe("CoveDB", () => {
  ...
  it("returns signed-out session without triggering interactive login", async () => {
    ...
    await expect(db.whoAmI()).rejects.toMatchObject<Partial<CoveDBError>>({
      code: "SIGNED_OUT",
    });
  ...
  it("ignores legacy username-prefixed remembered rows", async () => {
    const kv = new MapKv();
    await kv.set(
      "covedb:saved-row:v1:owner:current-row",
      JSON.stringify({
        id: "owner_row",
        collection: "rows",
        baseUrl: "https://worker.example",
  ...
  it("explains how to provide Puter when signIn is called without a backend", async () => {
    ...
    await expect(db.signIn()).rejects.toMatchObject<Partial<CoveDBError>>({
      code: "SIGNED_OUT",
  ...
  it("throws CoveDBError for API failures", async () => {
    const db = new CoveDB({
      schema: MINIMAL_SCHEMA,
      identityProvider: async () => ({ username: "owner" }),
  ...
  it("starts provisioning a host-scoped federation worker from constructor prewarm", async () => {
    ...
    expect(consoleInfo).toHaveBeenCalledWith(
      `[covedb] deploying federation worker ${expectedWorkerName} for owner on ${appHost} at runtime ${CLASSIC_WORKER_RUNTIME_ID}`,
    );
    expect(consoleInfo).toHaveBeenCalledWith(
      `[covedb] federation worker ready ${expectedWorkerName} runtime ${CLASSIC_WORKER_RUNTIME_ID} ${deployedWorkerBase}`,
    );
  ...
  it("redeploys when stored metadata has no current runtime id", async () => {
    ...
    await expect(kv.get(workerMetadataKey("runtime", hostHash))).resolves.toBe(CLASSIC_WORKER_RUNTIME_ID);
    expect(consoleInfo).toHaveBeenCalledWith(
      `[covedb] upgrading federation worker ${workerName} for owner on ${appHost} from runtime missing-runtime-id to ${CLASSIC_WORKER_RUNTIME_ID}`,
    );
    consoleInfo.mockRestore();
  });
