import { COVEDB_INVITE_TARGET_PARAM } from "@covedb/core";
import {
  createContext,
  type ReactNode,
  useContext,
...
import type { CoveDB } from "@covedb/core";
import type {
  AuthSession,
  AnyRowHandle,
  CollectionName,
...
import {
  ...
  makeRowKey,
  type LoadStatus,
  type CoveDBReactRuntime,
  type QueryRows,
  type ResourceController,
  type ResourceSnapshot,
  snapshots,
} from "./runtime";
import { CoveDBReactRuntime as Runtime } from "./runtime";
...
> extends UseHookOptions {
  ...
  url?: string | null;
  clearInviteParams?: boolean | ((url: URL) => string);
  loadSavedRow?: (row: AnyRowHandle<Schema>, db: CoveDB<Schema>) => Promise<TResult> | TResult;
  acceptInvite?: (inviteInput: string, db: CoveDB<Schema>) => Promise<TResult>;
...
export interface CoveDBProviderProps<Schema extends DbSchema> {
  children: ReactNode;
  db: CoveDB<Schema>;
...
const RuntimeContext = createContext<CoveDBReactRuntime<any> | null>(null);
...
function useRuntime<Schema extends DbSchema>(db?: CoveDB<Schema>): CoveDBReactRuntime<Schema> {
  const contextRuntime = useContext(RuntimeContext);
  if (db) {
    if (contextRuntime && contextRuntime.client === db) {
      return contextRuntime as CoveDBReactRuntime<Schema>;
    }
  ...
  if (!contextRuntime) {
    throw new Error("CoveDBProvider is missing.");
  }
...
function useSessionResource<Schema extends DbSchema>(
  runtime: CoveDBReactRuntime<Schema>,
  enabled: boolean,
...
function getInviteInputFromUrl(): string | null {
  ...
  if (url.searchParams.has(COVEDB_INVITE_TARGET_PARAM)) {
    return url.toString();
  }
...
export function CoveDBProvider<Schema extends DbSchema>({
  children,
  db,
  subscribeToActivity,
}: CoveDBProviderProps<Schema>) {

  const runtimeRef = useRef<CoveDBReactRuntime<Schema> | null>(null);
  if (
...
export function useCoveDB<Schema extends DbSchema>(): CoveDB<Schema> {
...
export function useCoveDBReady<Schema extends DbSchema>(
  db: CoveDB<Schema>,
...
export function useSession<Schema extends DbSchema>(
  ...
  return {
    ...
    async signIn(): Promise<CoveDBUser> {
...
export function useCurrentUser<Schema extends DbSchema>(
  db: CoveDB<Schema>,
  options: UseHookOptions = {},
): UseResourceResult<CoveDBUser> {
  const runtime = useRuntime(db);
  const session = useSessionResource(runtime, options.enabled ?? true);
  if (session.status === "error") {
    return makeResourceResult<CoveDBUser>({
      error: session.error,
      status: "error",
    }, session.refresh);
  }
