...
type MutationSubscribedClient<Schema extends DbSchema> = CoveDB<Schema> & {
...
function snapshotCurrentUser(value: CoveDBUser): string {
  return stableJsonStringify(value);
}
...
export class CoveDBReactRuntime<Schema extends DbSchema = DbSchema> {
...
const defaultRuntimes = new WeakMap<object, CoveDBReactRuntime<any>>();

export function getDefaultRuntime<Schema extends DbSchema>(
  client: CoveDB<Schema>,
): CoveDBReactRuntime<Schema> {
  const existing = defaultRuntimes.get(client as object);
  if (existing) {
    return existing as CoveDBReactRuntime<Schema>;
  }

  const runtime = new CoveDBReactRuntime(client);
