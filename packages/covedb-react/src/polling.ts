...
export type ActivitySubscriber = (notify: () => void) => (() => void) | void;
...
export function subscribeToBrowserActivity(markActivity: () => void): (() => void) | null {
...
export function createAdaptivePoller(options: AdaptivePollerOptions): AdaptivePoller {
  ...
  const scheduleNext = (scheduleOptions: { onlyIfEarlier?: boolean } = {}): void => {
    if (!running) {
      return;
    }
    ...
    if (scheduleOptions.onlyIfEarlier && nextRunAt !== null && nextRunAt <= candidateRunAt) {
      return;
    }
  ...
  void startRun();
  const unsubscribe = (options.subscribeToActivity ?? subscribeToBrowserActivity)?.(markActivity);
